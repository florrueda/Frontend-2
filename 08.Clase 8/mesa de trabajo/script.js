...
function armarHTML(titulo,imgUrl,descripcion,fecha,tipoNacional) {
    const plantilla = `
    <div class="noticia">
    <h2>${titulo}</h2>
    <img src="${imgUrl}"/>
...
for (let i = 0; i < noticias.length; i++) {
    ...
    armarHTML(noticia.titulo, noticia.imgUrl, noticia.descripcion, noticia.fecha,noticia.tipoNacional);
};


const noti = document.querySelectorAll('.noticia');
for (let i = 0; i < noticias.length; i++) {
    const noticia = noticias[i];
    if(noticia.tipoNacional === true ) {
        noti[i].classList.add('nacional')
    }
};