...
const albumesFavoritos = document.getElementById('cant-favoritos');
function marcarFavorito(id) {
    ...
    renderizarAlbumes()
    const i = document.querySelectorAll('.favorito').length
    albumesFavoritos.innerText = i + " favoritos"
}

/* Tarea!
    - Agregar la cantidad de albumes al parrafo de totales.
    - Agregar la cantidad de albumes favoritos al parrafo de favoritos. --> se agrega en la misma funcion del marcar favorito
*/

const albumesTotales = document.getElementById('cant-albums');
function contarAlbumes (){
    let i = 0;
    albumes.forEach(album => {
        i++
    })
    albumesTotales.innerText = i + " álbumes"
}
contarAlbumes()
